...
tic
%% importing data
A=importdata('/Users/vaughan/Desktop/BE_price.csv');
%% normalizing data
...
%       x7f2(in)=exp((-1)*((x(7)-cfzl(7))^2)/((sifzl(7))^2));
     ...
     figure
plot(datan)
title(' Electricity Price: Testing data');
ylabel('Normalized data value');
...
hold off
 ...
 %Compute the evaluation criteria for LLWNN 
yob=ynorig+errnorig;
t1=length(ynorig);
ymob=mean(yob);
sqterrtest=(errnorig.^2);
diff=yob-ymob;
sqtdiff=(diff.^2);
R=1-((sum(sqterrtest)./sum(sqtdiff)));
MSE=(1/t1)*(sum(sqterrtest));
LL1=(1/t1)*sum((log((ynorig./yob).^2)));
RMSE=(MSE)^(1/2)
MAE=(1/t1)*sum(abs(yob-ynorig))
MAPE=(1/t1)*(sum(abs(errnorig./yob)))*100
figure
hold on
plot(yob,'k');
plot(ynorig','r');
title(' LLWNN output versus original during testing ');
legend('original output','LLWNN output');
%save the output of LLWNN 
csvwrite('/Users/vaughan/Desktop/yob.csv',yob');
csvwrite('/Users/vaughan/Desktop/ynorig.csv',ynorig');

 %Compute the evaluation criteria for LLWNN based fuyyz system
 yobtot=Ynnet+errntot;
t2=length(Ynnet);
ymobtot=mean(yobtot);
sqterrtesttot=(errntot.^2);
difftot=yobtot-ymobtot;
sqtdifftot=(difftot.^2);
Rtot=1-((sum(sqterrtesttot)./sum(sqtdifftot)));
MSEtot=(1/t2)*(sum(sqterrtesttot));
LLtot=(1/t2)*sum((log((Ynnet./yobtot).^2)));
RMSEtot=(MSEtot)^(1/2)
MAEtot=(1/t2)*sum(abs(yobtot-Ynnet))
MAPEtot=(1/t2)*(sum(abs(errntot./yobtot)))*100
%save the output of LLWNN basel Fuzzy system 
csvwrite('/Users/vaughan/Desktop/Ynnet.csv',Ynnet');

figure
hold on
plot(yobtot,'k');
plot(Ynnet,'r');
title(' LLWNN_Fuzzy output versus original during testing ');
legend('original output','LLWNN_Fuzzy output');
